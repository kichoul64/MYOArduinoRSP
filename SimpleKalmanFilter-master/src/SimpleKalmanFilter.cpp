...
SimpleKalmanFilter::SimpleKalmanFilter(float mea_e, float est_e, float q,float pp)
{
  _err_measure=mea_e;
  _err_estimate=est_e;
  _q = q;
...
float SimpleKalmanFilter::updateEstimate(float mea)
{
  ...
  _err_estimate =  (1.0 - _kalman_gain)*_err_estimate + fabs(_last_estimate - _current_estimate) * _q/pow(_current_estimate,_pp);
...
void SimpleKalmanFilter::setProcesspow(float pp)
{
  _pp=pp;
}
